...
for r = 0 : block_height_num - 1
    for c = 0: block_width_num - 1
       %% 方向滤波器，对高斯滤波后的图像处理
        % 图像分块
        I_block = I(r * block_height + 1 : (r + 1) * block_height, c * block_width + 1 : (c + 1) * block_width);
        figure(72), subplot(2, 3, 1), imshow(I_block), title(['I ', num2str(size(I))]);
        g_block = gauss_filter(I_block, parameter_of_D0);
        % figure(73), subplot(2, 3, 1), imshow(g_block); title(['高斯滤波后的时域图像块', num2str(size(I_gauss_block))]);
        % 滤波
        [g_pimer_direction_filtered, EMax, index_max] = deblock_filter(g_block, fft_M, fft_N);
       ...
       %% sobel 梯度计算方法得到的主方向，分别对高斯滤波后和原始图像的图像处理
        % 计算梯度
        [pixel_number_gauss, pimer_direction_index_gauss, Gdir_gauss] = SobelFilter(g_block);
